...
<script>
  ...
  export default{
    data(){
      return {
        url:URL+"/findcs.json?domain=0&csType=0&isFirm=1",
        pageSize:7,
        loading: false,
        pageNo:1,
        footerHeight:"85",
    ...
    methods:{
      ...
      changeNav:function(index){
        ...
        ctx.navIndex=index;
        if(index=="all"){
          ctx.url=URL+"/findCsByTchPlace.json?domain=0&csType=0&isFirm=1&pageSize="+ctx.pageSize+"&orgId="+ctx.orgid
        }else{
          ctx.url=URL+"/findCsByTchPlace.json?domain=0&csType=0&isFirm=1&pageSize="+ctx.pageSize+"&searchkey="+index+"&orgId="+ctx.orgid
        }
...
<style scoped>
  ...
  .categoryList .categories{width:7rem;height:1rem;}
