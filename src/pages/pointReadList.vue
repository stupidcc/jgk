          ...
          <div class="latestWebCourse">
            <div class="categoryList">
              <div class="categories fl">
                ...
                <a :class="{current:navIndex==0}" @click="changeNav(0)">免费</a>
                <a :class="{current:navIndex==1}" @click="changeNav(1)">原创</a>
                <a :class="{current:navIndex=='cs_studynum'}" @click="changeNav('cs_studynum')"><span class="point"></span>排行</a>
              </div>
...
<script>
  ...
  export default{
      ...
      searchCategory:function(cgId,cgName){
        ...
        ctx.ifSearched=true;
        ctx.urlCg=URL+"/findcs.json?domain=0&csType=1&pageSize="+ctx.pageSize+"&type="+cgId
      ...
      searchCourses:function(){
        var ctx=this;
        ctx.ifSearched=true;
        ctx.$http.get(URL+"/findcs.json?searchkey="+ctx.searchText+"&pageSize="+ctx.pageSize).then(response=>{
          ctx.resultList=response.body.list;
        if(ctx.resultList.length>0){
          ctx.hasResult=true;
        }else{
      ...
      changeNav:function(index){
        ...
        if(index=="cs_studynum"){
          ctx.url=URL+"/findcs.json?domain=0&csType=1&pageSize="+ctx.pageSize+"&orderby="+index
        }else if(index=="all"){
          ctx.url=URL+"/findcs.json?domain=0&csType=1&pageSize="+ctx.pageSize
        }else{
          ctx.url=URL+"/findcs.json?domain=0&csType=1&pageNo=1&pageSize="+ctx.pageSize+"&payType="+index
        }
...
<style scoped>
  ...
  #searchPage .searchResult .resultTT{width:100%;height:0.65rem;line-height:1rem;color:#999;font-size:0.373rem;padding-left:0.36rem;}
