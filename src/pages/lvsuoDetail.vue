<template>
  <div class="view" id="content">
    <section>
       ...
       <tentrance-for-lvsuo :id="lvsuoObj.id"></tentrance-for-lvsuo>
       <div class="container">
      <div class="latestWebCourse">
        <div class="titleView">
          <span class="icon fl"></span>
          <span class="ttInfo fl">最新网课</span>
          <router-link class="moreP" :to="'/pointViewListForLvsuo/'+lvsuoObj.id">
      ...
      <div class="notice">
        <div class="titleView">
          ...
          <router-link class="moreP" :to="'/msshowListForLvsuo/'+lvsuoObj.id">
            <span class="fr moreIcon"></span>
            <span class="moreInfo fr">更多</span>
          </router-link>
        </div>
      ...
      <div class="trainDynamic">
        <div class="titleView">
          ...
          <router-link class="moreP" :to="'/dynamicListForLvsuo/'+lvsuoObj.id">
            <span class="fr moreIcon"></span>
            <span class="moreInfo fr">更多</span>
          </router-link>
        </div>
        <div class="contentView">
          <dynamic-list :dynamic-list="dynamicList" entry="lvsuo"  :lvsuoId="lvsuoObj.id"></dynamic-list>
        </div>
      </div>
      <div class="stuVoice">
        <div class="titleView">
    ...
    <div class="fixed-background"v-show="ifShowApplyInfo" @click="closeWindow"></div>
...
<script>
  ...
  import TentranceForLvsuo from "../components/TentranceForLvsuo.vue"
    ...
    mounted:function(){
      ...
      ctx.$http.get(URL+"/getlawyerfirm.json?id="+this.$route.params.id).then(response=>{
              ...
              ctx.orgid=response.body.orgid;
                /*最新网课*/
                ctx.$http.get(URL+"/findcs.json?domain=0&pageNo=1&pageSize=6&type=2&csType=0&isFirm=1&orgId="+ctx.orgid).then(response=>{
                  ctx.latestCourse=response.body.list;
              },response=>{})
                /*排行榜*/
                ctx.$http.get(URL+"/findlawyerfirm.json?domain=0&pageNo=1&pageSize=10&orderBy='id'&orgId="+ctx.orgid).then(response=>{
                  ctx.rankList=response.body.result.data;
              },response=>{})
                /*面授*/
                ctx.$http.get(URL+"/getFacetofaces.json?domain=0&pageNo=1&pageSize=3&orgType=3&orgId="+ctx.orgid).then(response=>{
                  ctx.mshowList=response.body.result.data;
              },response=>{})
                /*评论*/
                ctx.$http.get(URL+"/getCourseComments.json?domain=0&pageNo=1&pageSize=3&orgId="+ctx.orgid).then(response=>{
                  ctx.commentList=response.body.result.data;
              },response=>{})
                /*动态*/
                ctx.$http.get(URL+"/getDjLXNotices.json?domain=0&pageNo=1&pageSize=3&orgId="+ctx.orgid).then(response=>{
...
<style scoped>
  #content{position: relative;width:10rem;margin:0 auto 1.33rem auto;}
  .lvsuoTop{width:10rem;height:4rem;position: relative}
  .lvsuoTop .logo{width:2.1rem;height:2.1rem;position:absolute;bottom:0.2rem;left:0.3rem;border:2px solid #d2d2d2;background: url(http://image.zfwx.com/upload/1488953657867.jpg) no-repeat center center #fff;}
  .lvsuoTop .fengjing{width:10rem;height:2.853rem;position: relative;background: url(../assets/imgs/temporary/fengjing.jpg) center center;background-size: contain}
  .lvsuoTop .fengjing .lvsuoName{position:absolute;bottom:0.4rem;left:2.75rem;font-size:0.43rem;font-family:方正黑体简体;color:#fff;}
  .lvsuoTop .lvsuoInfo{width:10rem;height:1.147rem;padding:0.13rem 0 0.2rem 0;background-color: #f4f4f4 }
  .lvsuoTop .lvsuoInfo .listDiv{width:7.6rem;margin-left:2.4rem;height:100%;}
  .lvsuoTop .lvsuoInfo .listDiv .item{width:2.53rem;height:100%;}
  .lvsuoTop .lvsuoInfo .listDiv .item:not(:last-child){border-right:2px solid #dfdfdf}
  .listDiv .item p{width:100%;text-align:center;}
  .listDiv .item p.num{height:0.44rem;line-height:0.44rem;font-size:0.373rem;font-family: 方正黑体简体;color:#000;font-weight: bold}
  .listDiv .item p.label{height:0.4rem;line-height:0.4rem;font-size:0.267rem;color:#999;font-family: 方正黑体简体;}
  ...
  .imgCourseList{width:100%;height:8.8rem;padding:0.267rem 0 0 0.32rem;background-color: #fff}
  .imgCourseList li{width:4.25rem;height:4.4rem;margin-right:0.32rem;}
  ...
  .clauseContain{position: fixed;width:0;background-color:#cccccc;top:0;right:0;bottom:85px;z-index:1}
